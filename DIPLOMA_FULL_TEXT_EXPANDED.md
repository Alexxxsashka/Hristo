...
Детальний аналіз поточної бізнес-моделі «AS-IS» магазину Hristo Airsoft дозволив ідентифікувати ряд системних проблем, які негативно впливають на прибутковість та масштабованість підприємства. Основні деструктивні чинники розподілені за трьома векторами: операційним, інформаційним та клієнтським.

#### 1.2.1. Операційні та інформаційні дефіцити
На даний момент процеси обробки замовлень характеризуються високим ступенем фрагментації даних. Кожен підрозділ використовує власні інструменти (Google Sheets для складу, Telegram для менеджерів, паперові накладні для майстерні), що призводить до наступних наслідків:
- **Інформаційна асиметрія:** дані про фактичну наявність товару на складі оновлюються з затримкою у кілька годин, що спричиняє випадки «подвійного продажу» однієї одиниці товару різним клієнтам.
- **Висока вартість помилки:** ручна перевірка сумісності страйкбольного обвісу (наприклад, сумісність конкретного ців'я з певною моделлю репліки) базується виключно на пам'яті та досвіді конкретного менеджера. Помилка на цьому етапі веде до витрат на логістику повернення та втрати лояльності клієнта.
- **Відсутність історичності даних:** ручне ведення таблиць не дозволяє відстежувати життєвий цикл товару або замовлення, що унеможливлює проведення глибокого аудиту у разі виникнення суперечливих ситуацій.

#### 1.2.2. Порівняльний аналіз ефективності процесів
Для кількісної оцінки потенційного ефекту від впровадження автоматизованої системи було проведено порівняльний хронометраж основних бізнес-процесів (табл. 1.1).

**Таблиця 1.1. Кількісні показники ефективності переходу від моделі AS-IS до TO-BE**

| Показник / Процес | Модель AS-IS (ручна) | Модель TO-BE (автоматизована) | Коефіцієнт прискорення |
| :--- | :---: | :---: | :---: |
| Валідація сумісності модулів | 8–15 хв (пошук у PDF) | 0.1 сек (логіка слотів у 3D) | ~9000x |
| Ініціалізація та розрахунок кошика | 3–5 хв (ручний перерахунок) | Миттєво (Zustand logic) | 100% |
| Обробка платежу та фіскалізація | 10–20 хв (перевірка виписки) | Автоматично (Stripe/API) | ~95% |
| Генерація звітності з продажів | 180–240 хв (збір даних) | 3–5 сек (BI Dashboard) | ~2800x |
| Синхронізація складських залишків | 60 хв (ручна звірка) | Миттєво (DB Triggers) | 100% |

#### 1.2.3. Обґрунтування стратегічної необхідності автоматизації
Впровадження розроблюваної системи є не просто технічним оновленням, а необхідним кроком для цифрової трансформації бізнесу з таких причин:
1.  **Зменшення операційного навантаження:** Автоматизація рутинних операцій (генерація SKU, перевірка сумісності) звільняє до 70% робочого часу менеджерів, що дозволяє сконцентруватися на активних продажах та підтримці клієнтів.
2.  **Підвищення конверсії через візуалізацію:** 3D-конфігуратор виконує роль «віртуального продавця», який працює 24/7, надаючи клієнту впевненість у правильності вибору, що критично важливо для високого середнього чека у сфері Airsoft.
3.  **Забезпечення прозорості та контролю:** Впровадження Audit Logs та BI-аналітики дозволяє керівнику приймати рішення на основі реальних цифр, а не інтуїтивних припущень.
4.  **Відповідність стандартам безпеки:** Перехід на професійні платіжні шлюзи та JWT-авторизацію мінімізує ризики шахрайства та витоку персональних даних користувачів.

Таким чином, розробка та впровадження спеціалізованої e-commerce платформи є економічно виправданим та технологічно необхідним рішенням для забезпечення сталого розвитку магазину Hristo Airsoft.
...
graph TD
    subgraph Actors
        G[Гість]
        C[Зареєстрований Клієнт]
        A[Адміністратор]
        P[Stripe / Банк]
    end

    subgraph "Система Hristo Airsoft"
        %% Guest Actions
        UC1(Перегляд каталогу та пошук)
        UC2(3D-конфігурація та візуалізація)
        UC3(Автентифікація та реєстрація)

        %% Customer Actions
        UC4(Керування кошиком)
        UC5(Оформлення замовлення)
        UC6(Вибір методу та оплата)
        UC7(Перегляд історії та профілю)

        %% Admin Actions
        UC8(Керування каталогом та 3D-моделями)
        UC9(Генерація SKU варіацій)
        UC10(Аналіз BI-метрик та доходів)
        UC11(Моніторинг Audit & Inventory Logs)
        UC12(Керування замовленнями та статусами)
    end
...
Об’єктом автоматизації є комплексні бізнес-процеси спеціалізованого магазину страйкбольного спорядження Hristo Airsoft. На відміну від стандартних рішень електронної комерції, даний об’єкт має ряд специфічних характеристик, що зумовлюють архітектуру розроблюваної системи.

#### 2.3.1. Функціонально-організаційна специфіка
Магазин спеціалізується на реалізації тактичного обладнання, яке характеризується:
- **Високим ступенем кастомізації:** більшість товарів є модульними (гвинтівки, бронежилети), де кінцевий вигляд та ціна залежать від обраних компонентів.
- **Складною матрицею SKU:** один базовий товар може мати десятки варіацій (колір, матеріал, тип приводу), що потребує автоматизованого генератора комбінацій для точного складського обліку.
- **Нішевою цільовою аудиторією:** користувачі вимагають детальної технічної інформації та можливості візуальної перевірки сумісності деталей перед покупкою.

#### 2.3.2. Характеристика інформаційних потоків
Процес автоматизації охоплює повний життєвий цикл взаємодії з клієнтом, що розділяється на наступні рівні:
1.  **Інформаційно-візуальний рівень:** передача даних від 3D-движка (координати слотів, типи з'єднань) до клієнтського інтерфейсу для формування унікальної збірки.
2.  **Транзакційний рівень:** обмін даними між фронтендом та API для валідації кошика, застосування промокодів та ініціалізації платежів через Stripe або формування реквізитів для банківського переказу.
3.  **Логістично-адміністративний рівень:** автоматичне оновлення статусів замовлень, журналювання змін у запасах товарів та формування аналітичних звітів для власника бізнесу.

#### 2.3.3. Цілі та межі автоматизації
Автоматизація спрямована на перетворення магазину з пасивної вітрини в активну платформу підтримки прийняття рішень. Межі автоматизації включають:
- Перенесення процесів конфігурування з консультацій у чатах безпосередньо у браузер користувача.
- Заміну ручного контролю оплат на автоматизовану систему синхронізації з платіжними шлюзами.
- Впровадження централізованого Audit Log для контролю дій персоналу та прозорості бізнес-операцій.

Таким чином, об’єкт автоматизації являє собою динамічну систему, де критично важливим є узгодження візуального представлення товару з його технічними та логістичними параметрами у базі даних.

### 2.4. Вимоги до системи

Для забезпечення конкурентоспроможності та високої якості користувацького досвіду, до розроблюваної системи висуваються наступні комплексні вимоги.

#### 2.4.1. Вимоги до функціональних характеристик
Система повинна забезпечувати виконання таких ключових функцій:
- **Інтерактивна 3D-візуалізація:** рендеринг GLB-моделей спорядження з можливістю динамічної заміни деталей (модулів) через систему слотів (`slot_` та `mod_`).
- **Гнучке керування каталогом:** підтримка ієрархічних категорій, фільтрації за технічними характеристиками та автоматичної генерації SKU-варіацій.
- **Повноцінний e-commerce цикл:** функціонал кошика, інтеграція з платіжним шлюзом Stripe (онлайн) та підтримка офлайн-оплат (банківський переказ, COD).
- **Багатоканальна автентифікація:** підтримка входу через OAuth (Google, Apple), за номером телефону (SMS) та традиційний Email/Password.
- **Адміністративне керування:** панель управління контентом, моніторинг замовлень у реальному часі та модуль аналітики (BI) з графіками продажів.
- **Журналювання та аудит:** автоматичне фіксування всіх критичних дій адміністраторів (Audit Logs) та рухів товару на складі (Inventory Logs).

#### 2.4.2. Вимоги до надійності та продуктивності
- **Час відгуку:** завантаження основної сторінки не повинно перевищувати 2 секунд при середній швидкості з'єднання.
- **FPS у 3D-сцені:** стабільна частота кадрів не менше 30 FPS на середньостатистичних мобільних пристроях та десктопах.
- **Цілісність даних:** забезпечення транзакційності (ACID) при проведенні платежів та списанні залишків зі складу для виключення оверселінгу.
- **Відмовостійкість:** автоматичне відновлення після збоїв завдяки використанню Serverless-архітектури та хмарної бази даних Neon з підтримкою point-in-time recovery.

#### 2.4.3. Вимоги до безпеки та захисту інформації
- **Авторизація:** використання рольової моделі доступу (RBAC) з перевіркою JWT-токенів Firebase на рівні бекенд-middleware.
- **Захист даних:** шифрування трафіку через SSL/TLS, використання заголовків безпеки (Helmet.js) та захист від перебору паролів (Rate Limiting).
- **Фінансова безпека:** відповідність стандарту PCI DSS через делегування обробки банківських карт сервісу Stripe (без збереження CVV/PAN на сервері магазину).

#### 2.4.4. Вимоги до ергономіки та інтерфейсу
- **Адаптивність:** коректне відображення на пристроях з шириною екрану від 320px до 4K (Mobile First).
- **Доступність:** відповідність вимогам WCAG 2.1 AA (контрастність тексту, ARIA-атрибути для скрін-рідерів).
- **UX/UI стандарти:** підтримка світлої та темної тем, візуальні підказки при взаємодії з 3D-моделлю, плавна анімація переходів.

#### 2.4.5. Вимоги до сумісності та масштабованості
- **Кросбраузерність:** підтримка актуальних версій Chrome, Firefox, Safari, Edge та мобільних браузерів на базі Chromium та WebKit.
- **Масштабованість:** архітектура повинна підтримувати горизонтальне масштабування при зростанні трафіку без зміни коду (cloud-native approach).
- **SEO-оптимізація:** підтримка серверного рендерингу або динамічної генерації мета-тегів для коректної індексації товарів пошуковими системами.
...
| :--- | :--- | :--- | :--- |
| 1 | 3D-конфігурація та рендеринг | Координати слотів (Empty), GLB-моделі деталей, вибір користувача | Візуалізована збірка в реальному часі, динамічна ціна, JSON-конфігурація |
| 2 | Автоматична генерація варіацій | Матриця атрибутів (колір, матеріал, розмір), базовий SKU | Масив комбінованих SKU-позицій із залишками та унікальними цінами |
| 3 | Автентифікація та авторизація (RBAC) | Електронна пошта, пароль, Google OAuth-токен | JWT-токен доступу, роль користувача (customer/admin), об'єкт профілю |
| 4 | Повнотекстовий пошук та фільтрація | Текстовий запит (tsvector), ID категорії, атрибути фільтрації | Відсортований та відфільтрований список об'єктів товарів |
| 5 | Керування кошиком та станом | ID товару, SKU обраної варіації, кількість | Оновлений глобальний стан (Zustand), розрахунок проміжної суми та податків |
| 6 | Оформлення замовлення (Hybrid Payment) | Дані кошика, дані доставки (ПІБ, адреса), обраний метод (Stripe/Bank/COD) | Номер замовлення у БД, платіжна сесія Stripe, банківські реквізити або підтвердження |
| 7 | Керування контентом (Admin CMS) | Параметри товарів, файли зображень, 3D-файли (glb) | Оновлений каталог у PostgreSQL, завантажені файли у Vercel Blob, записи в Audit Log |
| 8 | BI-аналітика та візуалізація | Агреговані дані продажів, часовий інтервал | Інтерактивні SVG-графіки (Recharts), розрахунок KPI (AOV, Revenue, Conversion) |
| 9 | Синхронізація в реальному часі | Події зміни стану (BroadcastChannel API) | Оновлення UI в усіх відкритих вкладках браузера без перезавантаження |
| 10 | Зворотний зв'язок та контактна форма | ПІБ, email, тема повідомлення, текст звернення | Запис у таблиці messages, статус прочитання, сповіщення адміністратора |
